...
exports.DepCost = class DepCost extends EventEmitter {
  ...
  async processOptions() {
    ...
    let { recentVersions, versions } = options
    let finalPkgs = pkgs

    if (versions) {
      if (recentVersions) {
        log.warn(`"versions" option is enabled and "recentVersions" is disabled.`)
      }
      if (typeof versions === 'string') {
        versions = versions.split(',').map(v => v.trim()).filter(v => v)
      }
      const targetPkg = pkgs[0]
      finalPkgs = versions.map(version => `${targetPkg}@${version}`)

    } else if (recentVersions) {
      if (pkgs.length > 1) {
